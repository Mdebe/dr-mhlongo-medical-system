...
export function Services() {
  return (
    <section id="services" className="bg-white scroll-mt-24">
      <div className="mx-auto max-w- px-4 sm:px-6 lg:px-10 py-16 sm:py-20 lg:py-28">
        <div className="mb-10 sm:mb-14">
          <p className="text- sm:text- font-bold uppercase tracking-[0.24em] text-[#D9A24B] mb-4">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5">
            ...
            <p className="max-w- text- sm:text-[14.5px] leading-[1.6] text-[#0B1B33]/60 shrink-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
          {services.map(({ title, description, icon: Icon, href }) => (
            <Link
              key={title}
              href={href}
              className="group relative flex min-h- sm:min-h- flex-col overflow-hidden rounded- sm:rounded- border border-black/[0.06] bg-[#0B1B33] p-7 sm:p-8 transition-all duration-500 hover:-translate-y-1.5 hover:shadow-[0_24px_50px_-16px_rgba(11,27,51,0.35)]"
            >
              {/* Background image */}
              <div className="absolute inset-0 overflow-hidden">
                <div className="relative w-full h-full">
                  <Image
                    src={serviceImages[title] || "/images/services/joint.jpg"}
                    alt=""
                    fill
                    sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"
                    className="object-cover opacity-[0.28] group-hover:opacity-[0.38] group-hover:scale-[1.06] transition-all duration-700 ease-out"
                  />
                </div>
              ...
              <div className="relative z-10 flex h-full flex-col">
                ...
                <h3 className="mt-6 text- sm:text- font-bold leading-[1.15] tracking-[-0.02em] text-white">
                  {title}
                </h3>
                <p className="mt-2.5 line-clamp-3 text-[13.5px] sm:text- leading-[1.6] text-white/65 group-hover:text-white/80 transition-colors">
                  {description}
                </p>
          ...
          {/* CTA Card */}
          <div className="relative flex min-h- flex-col justify-center overflow-hidden rounded- sm:rounded- bg-[#F7F5F1] border border-black/[0.06] p-7 sm:p-8">
            <div className="absolute -right-16 -top-16 h- w- rounded-full bg-[#D9A24B]/15 blur- pointer-events-none" />
            <h3 className="relative text- sm:text- font-bold leading-[1.2] text-[#0B1B33] text-balance">
              Need a consultation at Emalahleni Private Hospital?
            </h3>
            <p className="relative mt-3 text-[13.5px] sm:text- leading-[1.6] text-[#0B1B33]/60">
